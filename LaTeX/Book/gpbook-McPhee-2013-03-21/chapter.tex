...
see, e.g., \citep{Burlacu:2013:GECCOcomp}
...
The runs explored here used \emph{Plush genomes}, which are linear genomes consisting of 
instructions paired with \emph{close counts}. The close counts are natural numbers 
indicating how many open code blocks should be closed after a given instruction. Several instructions
that humans typically use with code blocks, such as conditionals and loops,
...
system to evolve the desired ``end blocks''.
...
\marginpar{Remove editors from long lists.}
\marginpar{Remove URLs}
