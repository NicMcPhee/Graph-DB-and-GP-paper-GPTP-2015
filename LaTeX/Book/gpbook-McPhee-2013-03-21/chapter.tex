...
\abstract{
	...
	form that obscures or completely masks the profusion of specific events such as selections, crossovers, and mutations
	that are ultimately responsible for the aggregate behaviors that we end up discussing. In this chapter we take an almost
...
\marginpar{I think we'll remove the specific queries.}
%\marginpar{Should all the query details go in
...
\begin{table}%
	\centering
	\caption{Two examples of the impact of selection on evolutionary dynamics in the two explored runs. The table in \subref{table:branching} lists the number of parents contributing to a winning individual $n$ generations away for both the 
		...
		The table in~\subref{table:numChildren} lists the 20 most fecund individuals across the entirity of each of the lexicase and tournament selection runs.}
	\label{tab:big}
	\subtable[]{
	...
	\subtable[]{
		...
		\begin{tabular}{rrr}
			...
			Rank in run & $\quad$ Tournament & $\quad$ Lexicase  \\
			\hline\noalign{\smallskip}
			1 & 24 & 934 \\
			2 & 23 & 657 \\
			3 & 23 & 594 \\
