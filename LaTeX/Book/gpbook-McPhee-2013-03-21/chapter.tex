...
however,
don't lend themselves to the kinds of analysis that we want and need in evolutionary computation work see Section \ref{subsec:Neo4j}. %Most relational and document-based databases, for example require things like complex and expensive 
%recursive joins to trace significant hereditary lines, making this approach increasingly infeasible.
...
feasability/efficiency issues.) While we decided to utilize Neo4j as our graph database, there are other graph databases that would be an effective tool to use as well such as blank or blank. 
...
%40 rows
%766 ms
% tourneyment run 30 less selection pressure on one parent
%%match (n)-[* 1]->(m:individuals{generation: 142}) %return id(n), count(distinct m) order by %count(distinct m) desc limit 40;
%+----------------------------+
%| id(n)  | count(distinct m) |
%+----------------------------+
%| 141736 | 13                |
...
%neo4j-sh (?)$ match (n:individuals)-[* 1]-%>(m:individuals) return id(n), count(distinct m) %order by count(distinct m) desc limit 40;
%+----------------------------+
%| id(n)  | count(distinct m) |
%+----------------------------+
%| 21273  | 25                |
...
%26 rows
%1966949 ms
% match (n) where n.generation > 126 with distinct n.generation as gens unwind gens as g match (p:individuals{generation: g})-[*]->(c {total_error: 0}) return g, count(distinct p) order by toInt(g) %asc;
%+-------------------------+
%| g   | count(distinct p) |
%+-------------------------+
%| 127 | 212               |
