...
\begin{table}[t]
	...
	\begin{center}
	\begin{tabular}{rrr}
		Generations Away From Winner & $\quad$ Number of Tournament Parents & $\quad$ Number of Lexicase Parents  \\
		\hline\noalign{\smallskip}
		18 & 297 & 58\\
		17 & 236 & 52\\
		16 & 180 & 46\\
...
Another major difference was the selection pressure of the style of selections. In lexicase selection, one parent can dominate the selection if it performs well for a majority of the test cases as we saw in section~\ref{sec:surprisingFecundity} where individual 86:261 was a parent for all but  approximately 70 individuals in the final generation. However, tournament selection never can have this level of selection pressure. Throughout the entire run, see Table~\ref{table:numChildren}, a single parent only produced 24 children at maximum. Most of the other top producing parents produced children around the upper teens. Comparing this to the lexicase selection there were over 19 of the 20 top parents produced over 200 children. This is an extreme difference in selection pressure between the two types of selections. This also reinforces the property that tournaments tend to branch out quickly and have a larger number of contributing parents. 

Beside selection pressure, we noticed another crucial difference between the types of individuals selected for reproduction. With tournament selection, there is a biased towards individuals that have the lowest fitness. However, this is not the case in lexicase.  As long as an individual performs extremely well for many cases but not all, it is still possible to be selected for reproduction. In Table~ \ref{table:winnerFitnesses}, individual 85:086 has an error of 100,000. This late into the run, it would be very unlikely that tournament would select this individual to reproduce. In fact, we see this in the tournament run where there isn't a individual selected in the winning ancestry that has a total error of more than 132. Additionally, we noticed that within the last 20 generations of the parents that reproduced, there were only at most five distinct total errors within each generation. \marginpar{awkward wording... table?} Tracing the winning ancestry back six generation back, we found that ancestors back contained individuals with a total error of either 83 or 132. This seems to mean that tournament seems to keep slamming the best individuals together until it miraculously produces a child better than it whereas lexicase identifies qualities that are necessary to produce a better children and utilizes those parents. This might contribute to the fact that lexicase had much more successes than tournament. 
